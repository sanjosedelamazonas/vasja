...
public class RubroProyectoTable extends ResizeTableLayout  {

    	ExtendedJPAContainer<RubroProyecto> container;
	    MutableEntityProvider<RubroProyecto> myEntityProvider;
	    ...
	    public RubroProyectoTable(Window w) {
	    	super(w);
	    	table = new Table("Rubros proyectos") {
    			@Override
	    	...
	    	redraw();
	    	final RubroProyectoTable ut = this;
	        table.addActionHandler(new Action.Handler() {
	            ...
	            public void handleAction(Action action, Object sender, Object target) {
	            	if (ACTION_DEL == action) {
	                    ...
	                    final SubwindowModal sub = new SubwindowModal("Elminar rubro presupuestal");
	                    final EntityItem<RubroProyecto> eItem = container.getItem((Long)item.getItemProperty("id").getValue());
	                    Label message = new Label("?Eliminar rubro presupuestal" + item.getItemProperty("nombre").getValue() + "?");
	                	...
	                	Item item = table.getItem(target);
						SubwindowModal sub = new SubwindowModal("Editar rubro presupuestal");
						EntityItem<RubroProyecto> eItem = container.getItem((Long) item
								.getItemProperty("id").getValue());
						sub.getContent().addComponent(
								new RubroProyectoForm(eItem.getEntity(), ut));
						getWindow().addWindow(sub);
					} else if (ACTION_NEW == action) {
						SubwindowModal sub = new SubwindowModal("Nuevo rubro presupuestal");
						sub.getContent().addComponent(new RubroProyectoForm(ut));
						getWindow().addWindow(sub);
					}
	            }
	        });
				...
				public Field createField(Container container, Object itemId,
					...
					if ("categoriaRubroProyecto".equals(propertyId)) {
						...
						DataFilterUtil.bindComboBox(cmb, "nombre",
								"Seleccione categoria Rubro presupuestal de proyecto", RubroProyecto.class, f);
	    ...
	    public void redraw() {
	        ...
	        container = new ExtendedJPAContainer<RubroProyecto>(
	        		RubroProyecto.class);
            MutableLocalEntityProvider<RubroProyecto> myEntityProvider = ConfigurationUtil.getRubroProyectoEP();
            container.setEntityProvider(myEntityProvider);
    		container.addNestedContainerProperty("categoriaRubroProyecto.*");
    		container.addNestedContainerProperty("cuentaRubroProyecto.*");
            table.setContainerDataSource(container);

	        // turn on column reordering and collapsing
	        table.setColumnReorderingAllowed(true);
	        table.setColumnCollapsingAllowed(true);
	        table.setVisibleColumns(new String[] { "id", "codigo", "isLeaf", "level", "nombre", "cuentaRubroProyecto.numero", "categoriaRubroProyecto.nombre", "descripcion" });
	        // set column headers
    		...
    		table.setColumnWidth("categoriaRubroProyecto.nombre", 120);
	    }
}