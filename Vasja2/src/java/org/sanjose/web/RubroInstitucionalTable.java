...
public class RubroInstitucionalTable extends ResizeTableLayout  {

    	ExtendedJPAContainer<RubroInstitucional> container;
	    MutableEntityProvider<RubroInstitucional> myEntityProvider;
	    ...
	    public RubroInstitucionalTable(Window w) {
	    	super(w);
	    	table = new Table("Rubros institucionales") {
    			@Override
	    	...
	    	redraw();
	    	final RubroInstitucionalTable ut = this;
	        table.addActionHandler(new Action.Handler() {
	            ...
	            public void handleAction(Action action, Object sender, Object target) {
	            	if (ACTION_DEL == action) {
	                    ...
	                    final SubwindowModal sub = new SubwindowModal("Elminar rubro institucional");
	                    final EntityItem<RubroInstitucional> eItem = container.getItem((Long)item.getItemProperty("id").getValue());
	                    Label message = new Label("?Eliminar rubro institucional " + item.getItemProperty("nombre").getValue() + "?");
	                	...
	                	Item item = table.getItem(target);
						SubwindowModal sub = new SubwindowModal("Editar rubro institucional");
						EntityItem<RubroInstitucional> eItem = container.getItem((Long) item
								.getItemProperty("id").getValue());
						sub.getContent().addComponent(
								new RubroInstitucionalForm(eItem.getEntity(), ut));
						getWindow().addWindow(sub);
					} else if (ACTION_NEW == action) {
						SubwindowModal sub = new SubwindowModal("Nuevo rubro institucional");
						sub.getContent().addComponent(new RubroInstitucionalForm(ut));
						getWindow().addWindow(sub);
					}
	            }
	        });
				...
				public Field createField(Container container, Object itemId,
					...
					if ("categoriaRubroInstitucional".equals(propertyId)) {
						...
						DataFilterUtil.bindComboBox(cmb, "nombre",
								"Seleccione categoria Rubro Institucional", RubroInstitucional.class, f);
	    ...
	    public void redraw() {
	        ...
	        container = new ExtendedJPAContainer<RubroInstitucional>(
	        		RubroInstitucional.class);
            MutableLocalEntityProvider<RubroInstitucional> myEntityProvider = ConfigurationUtil.getRubroInstitucionalEP();
            container.setEntityProvider(myEntityProvider);
    		container.addNestedContainerProperty("categoriaRubroInstitucional.*");
            table.setContainerDataSource(container);

	        // turn on column reordering and collapsing
	        table.setColumnReorderingAllowed(true);
	        table.setColumnCollapsingAllowed(true);
	        table.setVisibleColumns(new String[] { "id", "codigo", "isLeaf", "level", "nombre", "categoriaRubroInstitucional.codigo", "categoriaRubroInstitucional.nombre", "descripcion" });
	        // set column headers
    		...
    		table.setColumnWidth("categoriaRubroInstitucional.nombre", 120);
	    }
}